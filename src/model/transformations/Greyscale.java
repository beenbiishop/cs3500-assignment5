...
public class Greyscale implements ImageTransformation {
  ...
  public Image transform(Image image) {
    ...
    for (int i = 0; i < height; i++) {
      for (int j = 0; j < width; j++) {
        newPixels[i][j] = greyscalePixel(oldPixels[i][j]);
      }
    }
  ...
  private Color greyscalePixel(Color pixel) {
    ...
    for (int j = 0; j < 3; j++) {
      redVal += pixel.getRed() * GREYSCALE_MATRIX[0][j];
      greenVal += pixel.getGreen() * GREYSCALE_MATRIX[1][j];
      blueVal += pixel.getBlue() * GREYSCALE_MATRIX[2][j];
    }
